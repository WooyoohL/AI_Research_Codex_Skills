...
4. `candidate-claim-set`
5. `research-move-router`
6. `claim-prior-work-triangulation`
7. `paper-unit-scope-gate`
8. `decision-value-experiment-planner`
...
```text
field map → signals → candidate claims → prior-work collision → residual contribution → paper unit → decision-value experiments → reviewer risks → paper shape
```
...
| a vague idea | `candidate-claim-set` |
| a concrete method idea | `candidate-claim-set` → `claim-prior-work-triangulation` |
| failed experiments or results | `decision-value-experiment-planner` → `candidate-claim-set` |
...
| Claims | 2–4 candidate claims with prediction and disproof |
...
6. Minimal experiments must preserve the problem essence; cheap invalid proxies are not acceptable.
7. Every decision-relevant milestone must checkpoint to `research_state/`.
8. Research moves are provisional until prior-work triangulation.
9. Novelty can be partially verified, not proven from memory.
10. Reviewer simulation diagnoses risks; it does not predict acceptance.
